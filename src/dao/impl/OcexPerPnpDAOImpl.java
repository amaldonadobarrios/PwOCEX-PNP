...
public class OcexPerPnpDAOImpl implements OcexPerPnpDAO {
	...
	public int update(OcexPerPnp persona) {
		...
		u.setApeMatPer(persona.getApeMatPer());
		u.setApePatPer(persona.getApePatPer());
		u.setCipPer(persona.getCipPer());
		u.setCorreoPnp(persona.getCorreoPnp());
		u.setCtaAhorPer(persona.getCtaAhorPer());
		u.setDniPer(persona.getDniPer());
		u.setDomPer(persona.getDomPer());
		u.setFecNac(persona.getFecNac());
		u.setFlagPerPnp("1");
		u.setGenPer(persona.getGenPer());
		u.setGradoPer(persona.getGradoPer());
		u.setNomPer(persona.getNomPer());
		u.setTelCelPer(persona.getTelCelPer());
		u.setNomUniPer(persona.getNomUniPer());
		em.merge(u);
		em.getTransaction().commit();
		em.close();
		emf.close();
